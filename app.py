...
KANA_DATA = {
    "清音": {
        "行模式": {
            ...
            "ら行": [("ら","ラ","ra"), ("り","里","ri"), ("る","路","ru"), ("れ","レ","re"), ("ろ","罗","ro")],
...
# --- 2. 旗舰级发音引擎 (针对 iOS/Android 强制物理唤醒) ---
def play_audio(text_input):
    try:
        def calibrate(t):
            # 锁定原音：针对“へ”和“は”使用片假名锚定
            anchors = {"は": "ハ", "へ": "ヘ。", "を": "ヲ"}
        ...
        # 终极物理唤醒：将音频转为 Base64 并使用 JS 直接在顶级上下文播放
        ...
        """
        st.components.v1.html(force_play_js, height=0, width=0)
    except:
        pass
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"你是一名中日传译专家。翻译词汇‘{u_in}’。注意‘大家’应翻译为‘我々’等正式语。输出纯JSON：word, reading, pos, level, pitch, sentences(3句含jp, kana, cn)。"
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "Japanese translator spécialiste."}, {"role": "user", "content": prompt}],
...
# --- 3. UI 布局 ---
st.set_page_config(page_title="FUSION Pro v11.0", layout="wide")
...
if menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("请输入中文词汇", placeholder="号召、大家")
    if u_in:
        ...
        if display:
            ...
            for i, s in enumerate(display.get('sentences', []), 1):
                st.markdown(f'<div class="card-item"><b>{i}. {s.get("jp")}</b><br>{s.get("cn")}</div>', unsafe_allow_html=True)
    ...
    for idx, item in enumerate(current_list):
        if item[0]:
            with cols[idx % 5]:
                ...
                if st.button("🔊", key=f"b_{top_cat}_{sub_cat}_{idx}"):
                    play_audio(item[0])
