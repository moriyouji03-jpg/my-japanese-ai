...
KANA_DATA = {
    ...
    "清音-段": {
        ...
        "う段": [("う","ウ","u"), ("く","库","ku"), ("す","ス","su"), ("つ","ツ","tsu"), ("ぬ","ヌ","nu"), ("ふ","フ","fu"), ("む","姆","mu"), ("ゆ","ユ","yu"), ("る","ル","ru")],
        "え段": [("え","エ","e"), ("け","ケ","ke"), ("せ","セ","se"), ("て","テ","te"), ("内","ネ","ne"), ("へ","ヘ","he"), ("め","メ","me"), ("れ","レ","re")],
        "お段": [("お","オ","o"), ("こ","コ","ko"), ("そ","苏","so"), ("特","ト","to"), ("の","诺","no"), ("ほ","ホ","ho"), ("も","莫","mo"), ("よ","ヨ","yo"), ("ろ","罗","ro"), ("を","ヲ","wo")]
    },
    "浊音/半浊音": {
        "が行": [("が","ガ","ga"), ("ぎ","ギ","gi"), ("ぐ","グ","gu"), ("げ","格","ge"), ("ご","ゴ","go")],
        "ざ行": [("ざ","扎","za"), ("じ","ジ","ji"), ("ず","ズ","zu"), ("ぜ","ゼ","ze"), ("ぞ","左","zo")],
...
def play_audio(text_input):
    ...
    try:
        ...
        # 物理注入音频，使用 UUID 确保连续点击有效且不产生全页面闪烁
...
# --- 3. UI 布局与样式 ---
st.set_page_config(page_title="FUSION Pro v5.0", layout="wide")
...
# --- 模块 B: 五十音实验室 (局部刷新) ---
    ...
    def render_kana_lab():
        ...
        if selected_tab in KANA_DATA:
            ...
            if st.button(f"🔊 节奏连读：{sub_cat}", use_container_width=True, key=f"run_{uuid.uuid4()}"):
    ...
    def render_weekly():
        ...
        for i, item in enumerate(WEEKLY_CONTENT, 1):
            with st.expander(f"第 {i} 句：{item['jp']}"):
                ...
                if st.button(f"🔊 播放标准朗读", key=f"p_wk_{i}_{uuid.uuid4()}"):
                    play_audio(item['jp'])
    render_weekly()
