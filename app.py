...
# 2. 头部视觉 (和服少女 👘)
st.markdown('<div class="header-container"><div style="color:#1E3A8A;font-size:1.8rem;font-weight:800;">FUSION 智能化日语学习系统 1.0</div><div style="font-size:35px;">👘</div></div>', unsafe_allow_html=True)
...
if target:
    if not st.session_state.last_result or st.session_state.last_result.get('input') != target:
        with st.spinner('检索中...'):
            try:
                p = f"Translate '{target}' to Japanese. Hallucination forbidden (鸭=アヒル). Return JSON: {{'word':'Kanji','reading':'Kana','pos':'part','level':'N3','pitch':'0','sentences':[{{'jp':'S','kana':'K','cn':'C','en':'E'}}]}}"
                comp = client.chat.completions.create(
                    model="llama-3.3-70b-versatile",
                    messages=[{"role":"system","content":"Strictly accurate Japanese expert. Return exactly 3 sentences."}, {"role":"user","content":p}],
...
if st.session_state.last_result:
    ...
    if sents:
        ...
        for i, s in enumerate(sents, 1):
            st.markdown(f'<div class="card-{i}"><div style="display:flex;"><div class="idx">{i}</div><div style="flex:1;"><div style="font-size:1.1rem;font-weight:bold;">{s.get("jp","")}</div><div style="color:#666;font-size:0.85rem;">{s.get("kana","")}</div><div style="color:#059669;font-weight:500;">🇨🇳 {s.get("cn","")}</div><div style="color:#2563EB;font-size:0.8rem;">🇺🇸 {s.get("en","")}</div></div></div></div>', unsafe_allow_html=True)
