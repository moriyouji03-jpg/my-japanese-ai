...
# --- 2. 核心语音引擎 (v6.6 稳定占位符版) ---
def play_audio(text_input):
    audio_placeholder = st.empty()
    try:
        def calibrate(t):
            # 锁定原音：针对 へ 和 は 使用片假名锚定，彻底解决助词音
            anchors = {"は": "ハ", "へ": "ヘ", "を": "ヲ"}
...
def get_expert_translation(u_in):
    try:
        # 显式配置付费 API，强制使用 gpt-4o 顶配模型
        client = OpenAI(
            api_key=st.secrets["NEW_API_KEY"], 
            base_url=st.secrets["NEW_BASE_URL"]
        )
        prompt = f"你是一名顶尖的中日传译专家。请将词汇‘{u_in}’翻译为日语，并给出最地道的表达。要求输出纯JSON格式，包含字段：word, reading, pos, level, pitch, sentences(3个对象，含jp, kana, cn)。"
        response = client.chat.completions.create(
            model="gpt-4o", # 强制锁定顶配模型，拒绝瞎翻
            messages=[{"role": "system", "content": "You are a professional Japanese translator."}, {"role": "user", "content": prompt}],
            response_format={"type": "json_object"},
            temperature=0.3 # 降低随机性，确保专业度
        )
        return json.loads(response.choices[0].message.content)
    except Exception as e:
        return {"word": "Error", "reading": str(e), "sentences": []}

# --- 3. UI 布局 (回归 v4.5 极速架构) ---
st.set_page_config(page_title="FUSION Pro v6.6", layout="wide")
...
with st.sidebar:
    st.title("FUSION Pro")
    # 彻底弃用 segmented_control，改回最稳健的 radio 导航
    menu = st.radio("功能模块", ["AI 词汇专家", "五十音实验室", "每周 7 句"], index=0)

# --- 模块 A: AI 词汇专家 (加固版) ---
if menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("请输入中文词汇", placeholder="落实、号召、椅子")
    ...
    if query:
        ...
        if display:
            ...
            for i, s in enumerate(sentences, 1):
                # 终极防御：使用 .get() 绝对防止 AttributeError
                st.markdown(f"""<div class="card-item">
                    <b>{i}. {s.get("jp", "")}</b><br><small>{s.get("kana", "")}</small><br>
...
# --- 模块 B: 五十音实验室 (修复卡死) ---
    ...
    st.header("五十音实验室")
    # 改回标准的 selectbox，确保内容瞬间加载，不显示“同步中”
    sub_cat = st.selectbox("选择分类", ["あ行", "か行", "さ行", "た行", "な行", "は行", "ま行", "や行", "ら行", "わ行", "ん", "あ段", "い段", "う段", "え段", "お段", "が行", "ざ行", "だ行", "ば行", "ぱ行"])
    
    # 查找选中的数据
    current_list = []
    for cat in KANA_DATA.values():
        if sub_cat in cat:
            current_list = cat[sub_cat]
            break
