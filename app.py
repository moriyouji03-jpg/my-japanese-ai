...
KANA_DATA = {
    ...
    "清音-段": {
        "あ段": [("あ","ア","a"), ("か","カ","ka"), ("さ","サ","sa"), ("た","塔","ta"), ("な","ナ","na"), ("は","ハ","ha"), ("ま","マ","ma"), ("や","亚","ya"), ("ら","拉","ra"), ("わ","ワ","wa")],
        "い段": [("い","イ","i"), ("き","キ","ki"), ("し","希","shi"), ("ち","七","chi"), ("に","尼","ni"), ("ひ","希","hi"), ("み","ミ","mi"), ("り","里","ri")],
    ...
    "拗音体系": {
        ...
        "半浊拗音": [("ぴゃ","ピャ","pya"), ("ぴゅ","ピュ","pyu"), ("ぴょ","ピょ","pyo")]
    }
}
...
def play_audio(text_input):
    # 建立独立音频轨道，确保 UI 静止不闪烁
    audio_track = st.empty()
    try:
        def calibrate(t):
            # 锁定原音：使用音标前置引导逻辑，彻底解决助词音
            if t == "へ": return "e, へ"
            if t == "は": return "ha, は"
            if t == "を": return "o, を"
            return t
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"专家翻译'{u_in}'。JSON结构：word, reading, pos, level, pitch, sentences(3句含jp, kana, cn)。"
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "顶尖专家。"}, {"role": "user", "content": prompt}],
...
# --- 3. UI 布局 ---
st.set_page_config(page_title="FUSION Pro v5.8", layout="wide")
...
with st.sidebar:
    st.title("FUSION Pro")
    menu = st.radio("功能模块", ["五十音实验室", "AI 词汇专家", "每周 7 句"], index=0)
...
if menu == "五十音实验室":
    ...
    if selected_tab in KANA_DATA:
        ...
        # 按钮 Key 保持稳定但内容触发刷新
        if st.button(f"🔊 节奏连读：{sub_cat}", use_container_width=True, key=f"all_{sub_cat}"):
        ...
        for idx, item in enumerate(current_list):
            if item[0]:
                with cols[idx % num_cols]:
                    st.markdown(f"""<div class="kana-card"><div class="hiragana">{item[0]}</div><div style="color:#64748B; font-size:0.9rem;">{item[1]}</div><div style="color:#3B82F6; font-weight:600;">{item[2]}</div></div>""", unsafe_allow_html=True)
                    # 动态种子 Key，彻底解决连点击失效
                    if st.button("🔊", key=f"v_{sub_cat}_{idx}_{random.randint(0,10)}", use_container_width=True):
                        play_audio(item[0])
...
# --- 模块：AI 词汇专家 ---
    ...
    if query:
        ...
        if display:
            st.markdown(f"""<div class="word-box" style="max-width:650px; margin:auto;"><h1 style="color:#1E3A8A; margin:0;">{display.get('word','')}</h1><p style="color:#3B82F6; font-size:1.2rem; font-weight:bold; margin:5px 0;">【{display.get('reading','')}】</p><div style="font-size:0.8rem; color:#64748B;">🏷️ {display.get('pos','')} | 🏆 {display.get('level','')} | 📈 {display.get('pitch','')}型</div></div>""", unsafe_allow_html=True)
            
            if st.button("🔊 播放单词正音", key=f"voc_p_{query}_{random.randint(0,10)}"):
            ...
            for i, s in enumerate(display.get('sentences', []), 1):
                ...
                if st.button(f"🔊 播放例句 {i}", key=f"snt_{query}_{i}_{random.randint(0,10)}"):
                    play_audio(s.get("jp",""))
...
# --- 模块：每周 7 句 ---
    ...
    for i, item in enumerate(WEEKLY_CONTENT, 1):
        with st.expander(f"第 {i} 句：{item['jp']}"):
            ...
            if st.button(f"🔊 播放标准朗读", key=f"wk_p_{i}_{random.randint(0,10)}"):
                play_audio(item['jp'])
