...
# --- 2. 语音引擎 (针对手机 Safari/Chrome 极致加固) ---
def play_audio(text_input):
    audio_placeholder = st.empty()
    try:
        def calibrate(t):
            # 锁定原音：针对“へ”和“は”使用片假名+句号，强制浏览器不判为助词
            anchors = {"は": "ハ。", "へ": "ヘ。", "を": "ヲ。"}
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"专家翻译'{u_in}'。输出纯JSON：word, reading, pos, level, pitch, sentences(3句含jp, kana, cn)。注意“大家”应处理为“我々”或“皆様”。"
...
# --- 3. UI 布局 ---
st.set_page_config(page_title="FUSION Pro v8.7", layout="wide")
...
if menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("输入中文 (按回车查询)", placeholder="落实、号召、大家")
