...
KANA_DATA = {
    ...
    "清音-段": {
        ...
        "い段": [("い","イ","i"), ("き","キ","ki"), ("し","シ","shi"), ("ち","チ","chi"), ("开","ニ","ni"), ("ひ","ヒ","hi"), ("み","ミ","mi"), ("り","リ","ri")],
        ...
        "お段": [("お","オ","o"), ("こ","コ","ko"), ("そ","ソ","so"), ("と","ト","to"), ("の","ノ","no"), ("ほ","ホ","ho"), ("も","モ","mo"), ("よ","ヨ","yo"), ("ろ","罗","ro")]
    },
    "浊音/半浊音": {
...
def get_expert_translation(u_in):
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "顶尖传译专家。只输出纯JSON。"}, {"role": "user", "content": prompt}],
...
# --- 3. UI 布局与样式 ---
st.set_page_config(page_title="FUSION Pro v5.1", layout="wide")
...
if menu == "五十音实验室":
    ...
    def render_kana_lab():
        ...
        if selected_tab in KANA_DATA:
            ...
            if st.button(f"🔊 节奏连读：{sub_cat}", use_container_width=True, key=str(uuid.uuid4())):
            ...
            for idx, item in enumerate(current_list):
                if item[0]:
                    with cols[idx % num_cols]:
                        ...
                        if st.button("🔊", key=f"btn_{sub_cat}_{idx}_{uuid.uuid4()}"):
                            play_audio(item[0])
    render_kana_lab()
        ...
        if query:
            ...
            if display:
                ...
                for i, s in enumerate(display.get('sentences', []), 1):
                    ...
                    if st.button(f"🔊 播放例句 {i}", key=f"snt_{i}_{uuid.uuid4()}"):
                        play_audio(s.get("jp",""))
    render_ai_vocab()
