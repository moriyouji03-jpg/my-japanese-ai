...
KANA_DATA = {
    ...
    "清音-段": {
        "あ段": [("あ","ア","a"), ("か","カ","ka"), ("さ","サ","sa"), ("た","タ","ta"), ("な","ナ","na"), ("は","ハ","ha"), ("ま","マ","ma"), ("ら","ラ","ra"), ("わ","ワ","wa")],
        ...
        "お段": [("お","オ","o"), ("こ","コ","ko"), ("そ","ソ","so"), ("と","ト","to"), ("の","诺","no"), ("ほ","ホ","ho"), ("も","モ","mo"), ("ろ","罗","ro")]
    },
    "浊音/半浊音": {
...
def play_audio(text_input):
    try:
        def calibrate(t):
            # 针对 は(ha) 和 へ(he) 采用“片假名强制锚定法”，彻底解决助词读音问题
            if t == "は": return "ハ。" 
            if t == "へ": return "ヘ。"
            return t
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"专家翻译'{u_in}'。JSON格式：word, reading, pos, level, pitch, sentences(3句)。"
...
# --- 3. UI 布局与样式 ---
st.set_page_config(page_title="FUSION Pro v4.1", layout="wide")
...
if menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("请输入中文词汇 (回车查询)", placeholder="落实、号召")
    ...
    if current_word:
        ...
        if display:
            st.markdown(f"""<div class="word-box" style="max-width:650px; margin:auto;">
                <h1 style="color:#1E3A8A; margin:0;">{display['word']}</h1>
                <p style="color:#3B82F6; font-size:1.2rem; font-weight:bold; margin:5px 0;">【{display['reading']}】</p>
                <div style="font-size:0.8rem; color:#64748B;">🏷️ {display['pos']} | 🏆 {display['level']} | 📈 {display['pitch']}型</div>
            ...
            for i, s in enumerate(display.get('sentences', []), 1):
                with st.container():
                    ...
                    if st.button(f"播放例句 {i}", key=f"sent_btn_{i}_{time.time()}"):
                        play_audio(s.get('jp', ''))
...
# --- 模块 B: 五十音实验室 ---
    ...
    if selected_tab in KANA_DATA:
        ...
        if st.button(f"🔊 节奏连读整个 【{sub_cat}】", use_container_width=True, key=f"run_{sub_cat}"):
...
# --- 模块 C: 每周 7 句 ---
    ...
    for i, item in enumerate(WEEKLY_CONTENT, 1):
        with st.expander(f"第 {i} 句：{item['jp']}"):
            ...
            if st.button(f"🔊 播放标准音", key=f"wk_p_{i}_{time.time()}"):
                play_audio(item['jp'])
