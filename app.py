...
KANA_DATA = {
    "清音": {
        "行模式": {
            ...
            "ら行": [("ら","ラ","ra"), ("り","里","ri"), ("る","ル","ru"), ("れ","レ","re"), ("ろ","罗","ro")],
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"专家翻译词汇‘{u_in}’。注意：如果是“大家”、“号召”等词，请给出地道的日企公文表达。输出纯JSON：word, reading, pos, level, pitch, sentences(3句含jp, kana, cn)。"
...
# --- 3. UI 布局 (还原经典海军蓝风格) ---
st.set_page_config(page_title="FUSION Pro v8.6", layout="wide")
...
if menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("请输入中文词汇 (回车查询)", placeholder="椅子、落实")
...
# --- 模块 B: 五十音实验室 (恢复物理灵敏度) ---
    ...
    st.header("五十音实验室")
    top_cat = st.radio("大类", ["清音", "浊音/半浊音"], horizontal=True)
    
    if top_cat == "清音":
        mode = st.radio("展示方式", ["行模式", "段模式"], horizontal=True)
    ...
    if st.button(f"🔊 节奏连读整个 {sub_cat}", use_container_width=True):
...
# --- 模块 C: 每周 7 句 ---
    ...
    for i, item in enumerate(WEEKLY_CONTENT, 1):
        with st.expander(f"第 {i} 句：{item['jp']}"):
            ...
            if st.button(f"🔊 朗读该句", key=f"wk_p_{i}"):
                play_audio(item['jp'])
