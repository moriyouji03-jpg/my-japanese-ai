...
KANA_DATA = {
    ...
    "清音-段": {
        ...
        "お段": [("お","オ","o"), ("こ","コ","ko"), ("そ","苏","so"), ("と","ト","to"), ("の","诺","no"), ("ほ","ホ","ho"), ("も","莫","mo"), ("よ","ヨ","yo"), ("ろ","罗","ro")]
    },
    "浊音/半浊音": {
...
def play_audio(text_input):
    try:
        ...
        # 物理注入音频组件，不再使用 st.empty 以免组件瞬间消失
...
# --- 3. UI 布局与样式 ---
st.set_page_config(page_title="FUSION Pro v4.8", layout="wide")
...
with st.sidebar:
    st.title("FUSION Pro")
    menu = st.radio("功能切换", ["AI 词汇专家", "五十音实验室", "每周 7 句"], index=1)
...
if menu == "五十音实验室":
    ...
    if selected_tab in KANA_DATA:
        ...
        # 使用随机数 Key 确保按钮点击永远有效
        if st.button(f"🔊 节奏连读：{sub_cat}", use_container_width=True, key=f"run_{sub_cat}_{random.randint(0,9999)}"):
        ...
        for idx, item in enumerate(current_list):
            if item[0]:
                with cols[idx % num_cols]:
                    ...
                    if st.button("🔊", key=f"v_{sub_cat}_{idx}_{random.randint(0,9999)}", use_container_width=True):
                        play_audio(item[0])
...
# --- 模块：AI 词汇专家 ---
    ...
    if query:
        ...
        if display:
            ...
            if st.button("🔊 播放单词正音", key=f"voc_p_{random.randint(0,9999)}"):
            ...
            for i, s in enumerate(display.get('sentences', []), 1):
                ...
                if st.button(f"🔊 播放例句 {i}", key=f"snt_btn_{i}_{random.randint(0,9999)}"):
                    play_audio(s.get('jp', ''))
...
# --- 模块：每周 7 句 ---
    ...
    for i, item in enumerate(WEEKLY_CONTENT, 1):
        with st.expander(f"第 {i} 句：{item['jp']}"):
            ...
            if st.button(f"🔊 播放朗读", key=f"wk_p_{i}_{random.randint(0,9999)}"):
                play_audio(item['jp'])
