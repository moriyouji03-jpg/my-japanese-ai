...
def get_fusion_business_pro(user_input):
    # 核心：通过更强的模型和更严谨的 Prompt 解决语义偏差
    prompt = f"""
    Translate Chinese '{user_input}' to Japanese.
    Target: JLPT N4/N5 learners.
    Rules:
    - Use common daily vocabulary (e.g., '工作' -> '仕事').
...
if st.button("查询", type="primary", use_container_width=True) or (u_in and st.session_state.get('last_query') != u_in):
    if u_in:
        with st.spinner('FUSION 商业引擎加速中...'):
...
if st.session_state.last_res and st.session_state.last_res.get('q') == u_in:
    ...
    if st.button(f"🔊 重新播放单词音", key="v_main", use_container_width=True):
        st.session_state.audio_config = {"text": f"这について、以下の日本語が考えられます。 {r['word']}", "key": st.session_state.audio_config["key"]+1}
...
if st.session_state.audio_config["text"]:
    try:
        ...
        st.session_state.audio_config["text"] = None # 播完重置
    except: pass
