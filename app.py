...
def get_fusion_expert_final(user_input):
    ...
    prompt = f"""
    Role: Professional Japanese Interpreter.
    Input: '{user_input}'.
    ...
    3. QUANTITY: Exactly 3 example sentences.
    ...
    try:
        ...
        comp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a native Japanese speaker. You hate literal translations from Chinese."},
...
u_in = st.text_input("", placeholder="输入中文词汇 (例如：名落孙山)...", key="main_input", label_visibility="collapsed")
