...
KANA_DATA = {
    ...
    "清音-段": {
        ...
        "お段": [("お","オ","o"), ("こ","コ","ko"), ("そ","ソ","so"), ("と","ト","to"), ("の","诺","no"), ("ほ","ホ","ho"), ("も","モ","mo"), ("よ","ヨ","yo"), ("ろ","罗","ro")]
    },
    "浊音/半浊音": {
...
# --- 2. 发音引擎核心 (复刻 v4.5 的稳定占位符逻辑) ---
def play_audio(text_input):
    # 物理占位符，这是 v4.5 成功的核心
    audio_placeholder = st.empty()
    try:
        def calibrate(t):
            # 锁定原音：使用片假名锚定，这是目前最稳定的 ha/he 方案
            anchors = {"は": "ハ", "へ": "ヘ", "を": "ヲ"}
...
def get_expert_translation(u_in):
    try:
        ...
        prompt = f"专家分析词汇‘{u_in}’。JSON结构：word, reading, pos, level, pitch, sentences(3句含jp, kana, cn)。"
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "顶尖传译专家，输出纯JSON。"}, {"role": "user", "content": prompt}],
            response_format={"type": "json_object"}
        )
        return json.loads(response.choices[0].message.content)
    except: return None

# --- 3. UI 布局与样式 (恢复 v4.5 的经典海军蓝) ---
st.set_page_config(page_title="FUSION Pro v5.7", layout="wide")
...
# --- 模块：AI 词汇专家 (v4.5 稳定架构) ---
elif menu == "AI 词汇专家":
    st.header("AI 词汇专家")
    u_in = st.text_input("请输入词汇", placeholder="落实、号召")
...
# --- 模块：每周 7 句 ---
    ...
    for i, item in enumerate(WEEKLY_CONTENT, 1):
        with st.expander(f"第 {i} 句：{item['jp']}"):
            ...
            if st.button(f"🔊 点击朗读", key=f"wk_{i}"):
                play_audio(item['jp'])
